...
`S6.1 — Define release/delivery observation boundary and G8`
...
| S6.1 | Define release/delivery observation boundary and G8 | AWAITING_REVIEW | RED→GREEN: 3 delivery tests; a healthy expected deployment completes G8; no delivery target marks G8 not applicable; a deployed SHA or environment mismatch fails G8; unauthenticated or agent actors are rejected; full `npm run check` with 182 tests; exact 100-artifact installed package | `GR-20260903-S6.1-R1`; pending commit |
