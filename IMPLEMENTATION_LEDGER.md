...
`S3.1 — Define path categories and surface catalog`
...
| S3.1 | Define path categories and surface catalog | AWAITING_REVIEW | RED→GREEN across R1 REJECT: provisional ids are digest-stable for any production path charset; 3 surface tests; untracked_ok docs vs uncertain production; unmatched production is provisional R2; proposal cannot drop hypotheses/coverage; full `npm run check` with 136 tests; exact 55-artifact installed package | `GR-20260902-S3.1-R2`; pending commit |
