...
`S3.6 — Implement surfaces explain and policy check CLI`
...
| S3.6 | Implement `surfaces explain` and `policy check` CLI | AWAITING_REVIEW | RED→GREEN: 2 surface CLI tests plus help/malformed updates; `surfaces explain` reports covering surface and path category; `policy check` reports weakening when a proposal drops hypotheses/risk and stays clean otherwise; full `npm run check` with 149 tests; exact 67-artifact installed package | `GR-20260902-S3.6-R1`; pending commit |
