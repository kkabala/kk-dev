...
`S2.7 — Implement protected gate run and advisory evidence show CLI`
...
| S2.7 | Implement protected `gate run` and advisory `evidence show` CLI | AWAITING_REVIEW | RED→GREEN: 3 gate CLI tests plus help/malformed updates; `--cmd`/caller argv cannot create evidence; `gate run` looks up a protected template by gate ID and writes authoritative measurements; `evidence show` is advisory and does not append; full `npm run check` with 133 tests; exact 52-artifact installed package | `GR-20260902-S2.7-R1`; pending commit |
