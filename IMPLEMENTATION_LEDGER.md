...
| S5.1 | Define GitHub provider boundary and authenticated state model | AWAITING_REVIEW | RED→GREEN: 3 github tests; authenticated GitHub observations parse into a frozen provider state; unauthenticated or agent actors are rejected; a merged SHA is accepted only when GitHub reports the pull request merged; full `npm run check` with 167 tests; exact 85-artifact installed package | `GR-20260903-S5.1-R1`; pending commit |
